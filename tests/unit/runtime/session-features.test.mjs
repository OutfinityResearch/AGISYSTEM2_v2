...
describe('Session Features', () => {
  ...
  describe('Load Command', () => {
    test('should load theory file', () => {
      ...
      session.learn('@_ Load "./evalSuite/suite07_transitive_reasoning/theories/taxonomy-hierarchy.sys2"');

      assert.ok(session.kbFacts.length > initialFacts, 'should have loaded facts');
    });
    ...
    test('should not double-load same file', () => {
      ...
      session.learn('@_ Load "./evalSuite/suite07_transitive_reasoning/theories/taxonomy-hierarchy.sys2"');
      const factsAfterFirst = session.kbFacts.length;

      session.learn('@_ Load "./evalSuite/suite07_transitive_reasoning/theories/taxonomy-hierarchy.sys2"');
      const factsAfterSecond = session.kbFacts.length;

      assert.equal(factsAfterSecond, factsAfterFirst, 'should not double-load');
    });
  });
